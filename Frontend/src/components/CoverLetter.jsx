...
export default function CoverLetterGenerator({ token }) {
    ...
    const handleDownloadPdf = (templateId) => {
        ...
        const { mT, mB, blh } = currentPageBreakInfo.pdfParams; 
        ...
        currentPageBreakInfo.profilePicPreviewRef = profilePicPreview; 
        ...
        if (actualBodyStartIndex !== -1) {
            letterBodyToPrint = generatedLetter.substring(actualBodyStartIndex).trimStart();
        } else {
            console.warn("PDF: Could not find salutation ending with ':' or ','. Attempting to find body start via double newline.");
            const dateAddressBlockEnd = accumulatedOffset; 
