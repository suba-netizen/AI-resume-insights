...
export default function CoverLetterGenerator({ token }) {
    ...
    // Define fetchHistory before it's used in useEffect
    ...
    // fetchHistory is a dependency, disable exhaustive-deps warning as it's a stable fetch function
    // eslint-disable-next-line react-hooks/exhaustive-deps
    useEffect(() => {
        if (token && showHistory) {
            fetchHistory();
        }
    }, [token, showHistory]); // Removed fetchHistory from dependency array, relying on disable comment
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.savedLetterId && saveLetter) {
                ...
                // This call is outside the useEffect dependency concerns
                if (showHistory) {
                    fetchHistory();
                }
            }
    ...
    const handleDownloadPdf = (templateId) => {
        ...
        currentPageBreakInfo.pdfParams = {
            ...
            mL_orig: 20, // eslint-disable-next-line no-unused-vars
            mR_orig: 20,
            mT: 20,
            mB: 20,
            blh: 7,
        ...
        const { mT, mB, blh } = currentPageBreakInfo.pdfParams; // eslint-disable-line no-unused-vars - Use mT, mB, blh locally. Suppress warning for this specific line.
        ...
        for (let i = 0; i < linesOfGeneratedLetter.length; i++) {
            ...
            if (isSalutationFlag && (lowerTrimmedLine.endsWith(':') || lowerTrimmedLine.endsWith(','))) { // Added parentheses here
